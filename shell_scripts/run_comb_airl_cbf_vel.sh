...
w_cbf=(1e-10 1e-8 1e-7 1e-6 1e-5 1e-4)
...
for w in "${w_cbf[@]}";
  do
    mainfolder="${FOLDER}v$i"
    python scripts/vel_train_airl_batch.py \
     --cbf_weight $w \
     --epoch_num 201 \
     --cbf_pth "data/new_vel/baselines/cbf" --log_pth "${mainfolder}/log" --share_pth "${mainfolder}/share" --airl_pth "${mainfolder}/airl"
    i=$(($i+1))
  done
