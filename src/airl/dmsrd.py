...
class DMSRD:
    ...
    def save_mixture_video(self, policies, iteration):
        ...
        for timestep in range(self.episode_length):
            act = np.dot(self.best_mixture, [policy.get_action(ob)[0] for policy in policies])
            act_executed = act
            ob, rew, done, info = self.env_test.step(act_executed)
            imgs.append(self.env_test.render('rgb_array'))
        save_video(imgs, os.path.join(f"{self.log_path}/policy_videos_{iteration}/new_mixture.mp4"))
    ...
    def save_video(self, iteration):
        ...
        for cluster_idx in range(len(self.mixture_weights)):
            ...
            for timestep in range(self.episode_length):
                ...
                imgs.append(self.env_test.render('rgb_array'))
            print(rews)
            save_video(imgs, os.path.join(f"{self.log_path}/policy_videos_{iteration}/mixture_{cluster_idx}.mp4"))
