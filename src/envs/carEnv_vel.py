...
class carEnv(Env):
    def __init__(self, demo='src/demonstrations/safe_demo_better.pkl', seed=10, is_hundred=False, is_test=False):
        ...
        with open(demo, 'rb') as f:
            ...
            for i, demo in enumerate(self.demonstrations):
                ...
                self.demonstrations[i] = demo

        # obs num + 1
        obs_num = self.demonstrations[0]['observations'][0].shape[0]

        # observation_space: (min(obs_num, TOP_K d_obs) + 1 agent) * [posx, posy]
    ...
    def step(self, action):
        ...
        self.agent_state = self.agent_state + dsdt * config.TIME_STEP  # yy: agent_state: [posx, posy], dsdt: [vx, vy]
    ...
    # YY: agent_state with shape (2), obs_states with shape (obs_num, 2)
