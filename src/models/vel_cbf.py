...
# yy: s_s [topk, 2], a_s [topk, 2]
def generate_unsafe_states_old(S_s, A_s, num_ratio=0.1, num_unsafe_state_each_frame=3):
    DIST_MIN_THRES = 0.06
    ...
    for i, s_u in enumerate(S_u_init):
        ...
        for j in range(topk):
            ...
            for _ in range(topk - j):  # select topk for the closest, topk - 1 for the 2nd closest, ...
                ...
                s_agent = np.array([s_agent[0] + x_bias, s_agent[1] + y_bias])
...
def get_action_batch(ob, policy, bs):
    ...
    # dist, mean, log_std = policy.extend(ob).outputs
    # samples = dist.sample()
    # return tf.reshape(samples, [bs, 1, 2])
...
def build_training_graph_init(num_obs):
    ...
    x = tf.expand_dims(s, 1) - tf.expand_dims(s, 0)  # yy: shape: [13, 13, 2]

    # Get h() from CBF NN (h shape: 1 * 13)
    h, mask = vel_core.network_cbf(x=x, r=config.DIST_MIN_THRES)
...
def build_training_graph_deriv(num_obs, policy):
    ...
    # Placeholders
    s = tf.placeholder(tf.float32, [min(num_obs + 1, config.TOP_K + 1), 2], name='ph_state')  # state
    s_next = tf.placeholder(tf.float32, [min(num_obs + 1, config.TOP_K + 1), 2],
    ...
    # h, mask = tf.stop_gradient(vel_core.network_cbf(x=x, r=config.DIST_MIN_THRES))  # TODO: check how to freeze
    h, mask = vel_core.network_cbf(x=x, r=config.DIST_MIN_THRES)

    # Get action via the policy
    # a = get_action_graph(tf.reshape(s, [1, 2 * min(num_obs + 1, config.TOP_K + 1)]), policy, istep)
    # a = get_action_graph(ob, policy)
    a = get_action_complex(tf.reshape(s, [1, 2 * min(num_obs + 1, config.TOP_K + 1)]), policy)
    ...
    loss_dang_deriv, loss_safe_deriv, acc_dang_deriv, acc_safe_deriv, num_dang, num_safe, unsafe_states = \
        vel_core.loss_derivatives(s, a, h, s_next, r=config.DIST_MIN_THRES, alpha=config.ALPHA_CBF)
...
def train_init_CBF_NN(demo_path,
    ...
    # Goal state
    goal_state = demonstrations[0]['observations'][-1][-1, :2]
...
# TODO: change to bs version -> change to (bs, s) as input to the CBF NN
def train_init_CBF_NN_new(demo_path,
    ...
    # # Get set of states and actions from demonstrations (yy: acceleration based)
...
# yy: Below are batch size base -----------------------------------------------------------------------------
def build_training_graph_deriv_batch(s, s_next, num_obs, policy, batch_size):
    """
    Description:
        Train policy to be safer. Freeze CBF NN and only minimizing loss_deriv.
    ...
    """
    # # Placeholders
    # s = tf.placeholder(tf.float32, [batch_size, min(num_obs + 1, config.TOP_K + 1), 2], name='ph_state')  # state
    # s_next = tf.placeholder(tf.float32, [batch_size, min(num_obs + 1, config.TOP_K + 1), 2],
    #                         name='ph_state_next')  # next timestep state

    # x is difference between the state of each agent and other agents
    x = tf.expand_dims(s, 2) - tf.expand_dims(s, 1)  # yy: shape: [bs, 13, 13, 2]

    # Get h() from CBF NN (h shape: 1 * 13). yy: freeze CBF NN here!!
    # h, mask = tf.stop_gradient(vel_core.network_cbf(x=x, r=config.DIST_MIN_THRES))  # TODO: check how to freeze
    # h, mask = vel_core.network_cbf(x=x, r=config.DIST_MIN_THRES)
    h = tf.compat.v1.map_fn(fn=lambda t: tf.reshape(vel_core.network_cbf(x=t, r=config.DIST_MIN_THRES)[0], [-1]), elems=x)  # [bs, 13]

    # Get action via the policy
    a = get_action_batch(tf.reshape(s, [batch_size, 1, 2 * min(num_obs + 1, config.TOP_K + 1)]), policy, batch_size)

    # Get loss and accuracy for derivative part
    loss, acc, h_min, h_next_min, deriv_reshape, deriv_safe, deriv_dang = \
        vel_core.loss_derivatives_min_batch(s, a, h, s_next, r=config.DIST_MIN_THRES, alpha=config.ALPHA_CBF)
...
if __name__ == '__main__':
    ...
    #                   log_path="data/comb/baselines_repro/log",
    #                   cbf_save_path="data/comb/baselines_repro/cbf",
    #                   num_obs=16,
    #                   is_load_unsafe_states=False,
