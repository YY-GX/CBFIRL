...
def build_training_graph(num_agents):
    ...
    #  YY: this is what we need to replace
...
def main():
    ...
    s, g, a, loss_list, loss, acc_list = build_training_graph(args.num_agents + 1)
    ...
    with tf.Session() as sess:
        ...
        for istep in range(config.TRAIN_STEPS):
            ...
            for i in range(accumulation_steps):
                ...
                a_np, out = sess.run([a, accumulate_ops], feed_dict={s: s_np, g: g_np})  # YY: when sess.run(accumulate_ops), the grad will accumulate in line 48, and later the accumulated grad will be used to optimize in line 67 & 68
