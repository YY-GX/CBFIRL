...
def build_evaluation_graph(num_agents):
    ...
    dsdt = core.dynamics(s, a_opt)  # YY: a_opt is the action with shape (N, 2)
    s_next = s + dsdt * config.TIME_STEP
    x_next = tf.expand_dims(s_next, 1) - tf.expand_dims(s_next, 0)
...
def main():
    args = parse_args()
    s, g, a, loss_list, acc_list = build_evaluation_graph(args.num_agents + 1)
    ...
    safety_single_agent = []  # YY: safety of our agent
    ...
    # TODO YY: Save start & goal points


    # YY: Save observation (s_np) & action (a_network)
    demostrations = []

    # TODO YY: Save unsafe states (how to define vx, vy of unsafe states?)
    unsafe_states = []
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        # YY: demo
        demostration = {'observations': [], 'actions': []}


        # YY: this loop is used to generate one traj
        # run INNER_LOOPS steps to reach the current goals
        for i in range(config.INNER_LOOPS):
            # compute the control input
            a_network, acc_list_np = sess.run([a, acc_list], feed_dict={s: s_np, g: g_np})  # YY: a_network is the action

            # YY log
            # print(i, s_np[-1, :], g_np[-1, :], a_network[-1, :])

            # YY: Add states and actions
            demostration['observations'].append(s_np)
            demostration['actions'].append(a_network[-1, :])
            ...
            dsdt = np.concatenate([s_np[:, 2:], a_network], axis=1)  # YY: dsdt = [vx, vy, ax, ay]
            # simulate the system for one step
            s_np = s_np + dsdt * config.TIME_STEP
            s_np_ours.append(s_np)
            safe_ratio_mask = core.ttc_dangerous_mask_np(
                s_np, config.DIST_MIN_CHECK, config.TIME_TO_COLLISION_CHECK)
            safety_ratio = 1 - np.mean(safe_ratio_mask, axis=1)
            safety_ours.append(safety_ratio)
            safety_info.append((safety_ratio == 1).astype(np.float32).reshape((1, -1)))

            safety_single_agent.append(safety_ratio[-1, 0])  # YY
            ...
            if args.vis:
                # YY: Only consider our agent (i.e., the last one)

                if np.linalg.norm(s_np[-1, :2] - g_np[-1, :]) < config.DIST_MIN_CHECK:
                    break
        ...
        # YY: add to demostrations
        demostrations.append(demostration)
        ...
        for i in range(config.INNER_LOOPS):
            ...
            # print('YY: ', core.ttc_dangerous_mask_np(s_np, config.DIST_MIN_CHECK, config.TIME_TO_COLLISION_CHECK).shape)
        ...
        if args.vis:
            ...
            for j in range(max(len(s_np_ours), len(s_np_lqr))):
                ...
                plt.scatter(s_np[:-1, 0], s_np[:-1, 1],
                            color='darkorange', 
                            s=agent_size, label='Dynamic Obstacle', alpha=0.6)
                # YY: our agent set to green
                plt.scatter(s_np[-1, 0], s_np[-1, 1],
                            color='green',
                            s=agent_size, label='Agent', alpha=0.6)
                # YY: Not display goal points for dynamic obstacles, but show for the agent
                plt.scatter(g_np[-1, 0], g_np[-1, 1],
                            color='deepskyblue',
                ...
                ax.set_xticklabels([])
                ax.set_yticklabels([])
                plt.legend(loc='upper right', fontsize=14, bbox_to_anchor=(1.15, 1.0))

                # plt.title('Agent: Safety Rate = {:.3f}'.format(
                #     np.mean(safety_single_agent)), fontsize=14)  # YY: safety of our agent
    ...
    demo_flie = open('src/demonstrations/safe_demo_perfect.pkl', 'wb')
