...
def network_action(s, g, obs_radius=1.0, indices=None):
    ...
    # YY: guess to be the element-wise ReLU activation function
    x = tf.contrib.layers.conv1d(inputs=x, 
                                 num_outputs=64,
                                 kernel_size=1, 
                                 reuse=tf.AUTO_REUSE,
    ...
    # YY: guess to be RowMax() "takes a matrix as input and outputs the maximum value of each row."
    x = tf.reduce_max(x * mask, axis=1)

    # YY: guess to be the concatenation operation in Figure 2
...
# YY: this is the rest part in loss function
...
# YY: this is the rest part in loss function
def loss_barrier_flex(h, s, r, ttc, indicator, indices=None, eps=[1e-3, 0]):
    """ Build the loss function for the control barrier functions.
